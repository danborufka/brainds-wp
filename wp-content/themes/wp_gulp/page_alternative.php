...
<?php 
	...
	// var_dump($siblings[0]);
		...
		foreach ( $siblings as $index => $item ) {
			if ( get_the_ID() == $item->ID ) {     //Current item
		...
		foreach ( $siblings as $index => $item ) {
			...
			if ( in_array( $on, get_post_ancestors( $item->ID ) ) && count(get_post_ancestors($on)) <= 3 ) {   // Child elemente
                $submenu =  (count(get_post_ancestors( $item->ID )) > count(get_post_ancestors($on))+1) ? "submenu" : "";


                if (count(get_post_ancestors( $item->ID )) > count(get_post_ancestors($on))+2) {
                    continue;   //Don't snow sub-sub-Categories
                }

				if ( get_the_ID() == $item->ID ) {
					echo '<li class="active '. $submenu . '">' . $item->post_title . '</li>';
				} else {
					echo '<li class="'. $submenu . '"><a href="' . get_permalink($item->ID) . '">' . $item->post_title . '</a></li>';
				}
			}
			//echo $item->post_title . ": " . $item->menu_item_parent . " - " . $item->post_parent . " - " . $item->ID . "<br>\n"; //
		}
